...
def apply_custom_css(is_dark: bool) -> None:
...
def render_header(
    ...
    st.markdown(
        ...
        <div class="pill-row">
            ...
            <div class="pill">Rerank: {"On" if use_rerank else "Off"}</div>
            <div class="pill">Retrieve: {modality_filter}</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
def render_context_panel(context_docs: list[dict]) -> None:
    if not context_docs:
        return
...
def main() -> None:
    ...
    render_header(engine, model=model, use_rerank=use_rerank, modality_filter=modality_filter)
    ...
    if process_files:
        if not groq_api_key or not jina_api_key:
            ...
            with st.spinner("Processing files and building index..."):
                added, skipped = process_uploaded_files(engine, uploads, audio_ok, audio_status)

                if engine.chunks:
                    engine.build_index()
                    st.success(
                        f"Indexed successfully. Added {added} chunks from {len(uploads)} file(s). "
    ...
    for message in st.session_state.chat_history:
        with st.chat_message(message["role"]):
            ...
            render_context_panel(message.get("context") or [])
    ...
    if user_query:
        ...
        with st.chat_message("assistant"):
            ...
            st.markdown(answer)
            render_context_panel(context_docs)
