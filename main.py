...
class AdvanceRAG:
    ...
    def build_index(self) -> None:
        ...
        valid_indices = [
            ...
            if chunk.modality != "image" or bool(chunk.meta.get("image_b64"))
        ]
        if not valid_indices:
            raise ValueError("No valid chunks with embeddable content.")

        text_indices = [idx for idx in valid_indices if self.chunks[idx].modality == "text"]
        image_indices = [idx for idx in valid_indices if self.chunks[idx].modality == "image"]

        embeddings: list[list[float] | None] = [None] * len(self.chunks)
        ...
        if any(emb is None for emb in embeddings):
            ...
            if missing == len(embeddings):
                raise ValueError("Embedding generation failed for all chunks.")

        filtered_embeddings = [
            emb
            for idx, emb in enumerate(embeddings)
            if idx in valid_indices and emb is not None
        ]
        emb_array = np.asarray(filtered_embeddings, dtype="float32")
    ...
    def retrieve(
        ...
        search_k = min(len(self._index_lookup), max(k, k * 4))
        _, indices = self.index.search(q_vec, search_k)

        results: list[ChunkRecord] = []
        for idx in indices[0]:
            if idx == -1:
                continue
            mapped_idx = self._index_lookup[int(idx)] if self._index_lookup else int(idx)
            record = self.chunks[mapped_idx]
    ...
    def _answer_with_vision(
        ...
        for doc in docs:
            ...
            if not image_b64:
                continue
            image_mime = doc.meta.get("mime", "image/png")
            content.append(
                {
                    "type": "input_image",
